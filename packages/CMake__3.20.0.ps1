Add-PackageInfo `
    ...
    -Version "3.20.0" `
    -Platform "x86_64" `
    -Url "https://github.com/Kitware/CMake/releases/download/v3.20.0/cmake-3.20.0-windows-x86_64.msi" `
    -FileName "from_url" `
    -DependsOn @("Env_config") `
    -RequiresElevatedPS $true `
    -FindCmd {
        where.exe cmake 2>&1 | Out-Null
        if (-not $? -or (cmake --version | Select-String '\b3\.20\.0\b' -Quiet) -ne $true) {
            throw 'Not found'
        }
    } `
    -InstallCmd {
